...
/**
 * @file ExprNodeType.h
 * @brief Содержит перечисление типов узлов синтаксического дерева.
 */

/**
 * @brief Перечисление типов узлов для построения дерева выражения.
 *
 * Включает в себя операнды (переменные, константы), арифметические,
 * логические, побитовые операции, а также узлы неявного приведения типов.
 */
enum ExprNodeType{
    NODE_UNKNOWN,                      ///< Неизвестный узел (по умолчанию)
    ...
    NODE_INTEGER_DIVISION,             ///< Целочисленное деление (div или //)
    ...
    NODE_CONVERT                       ///< Узел неявного преобразования типа (convert)
};
